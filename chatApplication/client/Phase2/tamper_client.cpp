...
bool aes_decrypt(const std::string& b64_input, const unsigned char* key, std::string& decrypted_out) {
    ...
    // If tampering occurred, EVP_DecryptFinal_ex will return <= 0
...
int main() {
    ...
    std::cout << "Connected. Initiating Diffie-Hellman Handshake...\n";
    ...
    bool trigger_tampering = false; // State flag for tampering
    ...
    while (true) {
        ...
        if (FD_ISSET(STDIN_FILENO, &readfds)) {
            ...
            if (b_read > 0) {
                ...
                // --- NEW COMMAND: INTERCEPT NEXT PACKET ---
                if (input == "/tamper") {
                    trigger_tampering = true;
                    std::cout << "[!] TAMPER MODE ARMED: The next incoming message will have one byte altered.\n";
                    continue;
                }
        ...
        if (FD_ISSET(sock, &readfds)) {
            ...
            while ((pos = rx_buffer.find('\n')) != std::string::npos) {
                ...
                // --- TAMPERING EXECUTION ---
                if (trigger_tampering) {
                    std::cout << "\n--- INTERCEPTED INCOMING PACKET ---\n";
                    std::cout << "Original Base64 Payload: " << frame << "\n";
                    
                    // Flip a character safely inside the base64 string (avoids padding characters)
                    if (frame.length() > 10) {
                        frame[10] = (frame[10] == 'A') ? 'B' : 'A';
                    }
                    
                    std::cout << "Tampered Base64 Payload: " << frame << "\n";
                    std::cout << "Attempting AES-GCM Decryption...\n";
                    trigger_tampering = false; // Disarm after single use
                }
                ...
                if (aes_decrypt(frame, aes_key, decrypted)) {
                    ...
                    // OpenSSL's EVP_DecryptFinal_ex detects the bad MAC and fails safely
