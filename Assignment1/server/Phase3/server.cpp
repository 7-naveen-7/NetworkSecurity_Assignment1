...
int main() {
    // ---------------------------------------------------------
    // PKI Setup: Load Server Private Key and Certificate String
    // ---------------------------------------------------------
    std::string server_cert_str = read_file("server_cert.pem");
    if (server_cert_str.empty()) {
        std::cerr << "Failed to read server_cert.pem\n";
        return -1;
    }

    FILE* key_file = fopen("server_key.pem", "r");
    if (!key_file) {
        std::cerr << "Failed to open server_key.pem\n";
        return -1;
    }
    ...
    while (true) {
        ...
        if (FD_ISSET(server_fd, &readfds)) {
            ...
            // PHASE 3: Immediately send the server certificate upon connection
            send_all(new_socket, server_cert_str);
            std::cout << "New connection. Certificate sent. Awaiting Challenge...\n";
        }
        ...
        for (auto it = clients.begin(); it != clients.end();) {
            ...
            if (FD_ISSET(sd, &readfds)) {
                ...
                while ((pos = it->second.rx_buffer.find('\n')) != std::string::npos) {
                    ...
                    // State 1: Awaiting Proof of Possession Challenge (Nonce)
                    if (it->second.state == AuthState::WAIT_CHALLENGE) {
                        std::string challenge = b64_decode(frame);
                        
                        // Sign the challenge using the server's private key
                        ...
                        std::string response = b64_encode(sig_str) + "\n";
                        send_all(sd, response);
                        
                        it->second.state = AuthState::WAIT_DH;
                        std::cout << "[Session " << sd << "] Signed challenge. Awaiting DH Key...\n";
                    }
                    // State 2: Diffie-Hellman Exchange
                        ...
                        std::cout << "[Session " << sd << "] DH Complete. Fingerprint: " << get_fingerprint(it->second.aes_key) << "\n";
